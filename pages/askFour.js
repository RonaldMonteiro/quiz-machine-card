...
export default function AskOne() {
  ...
  async function QuestionOne() {
    const data = "1";
    const response = await axios.get(`/api/answerFour?word=${data}`);
    const answer = response.data;
    localStorage.setItem("4", answer);
    router.push("/askFour");

    const one = localStorage.getItem("1").split(",");
    const two = localStorage.getItem("2").split(",");
    const three = localStorage.getItem("3").split(",");
    const four = localStorage.getItem("4").split(",");
   

    const arrayString = JSON.stringify([one, two, three, four])
    const responseResult = await axios.get(`/api/result?word=${arrayString}`);
    const answerResult = responseResult.data;

    router.push({
      pathname: "/result",
      query: { name: answerResult },
    });
  }
  ...
  return (
    <div className="flex flex-row w-full h-screen bg-[#2F3997] overflow-hidden">
      <Head>
        <title>Maquininha Ideal | 4ª Questão</title>
        <meta name="description" content="Quarta questão do quiz maquininha ideal"/>
      ...
      <div className="flex absolute flex-col w-full h-full overflow-hidden justify-center items-center">
        ...
        {/* ---------- AQUI VOCE EDITA A QUARTA PERGUNTA ---------- */}
          ...
          <p>
            ...
            <br /> {/* ==> aqui quebra a linha*/}
            Como seus clientes gostam de receber o o comprovante de pagamento?
          </p>
        ...
        {/* ---------- AQUI VOCE EDITA AS RESPOSTAS DA QUARTA PERGUNTA ---------- */}

        <div className={$.card_task} onClick={QuestionOne}>
          <h3>
            Impresso, eles se sentem mais seguros em ter o comprovante em
            mãos...
          </h3>
        </div>
        ...
        <div className={$.card_task} onClick={QuestionTwo}>
          <h3>
            SMS, é prático, dispensa papel e o cliente recebe em poucos
            segundos...
          </h3>
        </div>
